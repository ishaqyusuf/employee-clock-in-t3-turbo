...
import { randomNumber2, timeout } from "@acme/utils";
      ...
      <DataTableFacetedFilter
        ...
        options={_options()?.map((opt) =>
          typeof opt === "string"
            ? { label: opt, value: opt }
            : {
                label: labelKeyFn ? labelKeyFn(opt) : opt[labelKey],
                value: valueKeyFn ? valueKeyFn(opt) : opt[valueKey],
              },
        )}
      />
    </>
...
function ActionBtn({ size = "sm", ...props }: ActionBtnProps) {
  ...
  const Content = ({}) => (
    <>
      {Icon && <Icon className={cn("h-4 w-4", props.label ? "mr-2" : "")} />}
      {props.label}
    </>
  );
  return (
